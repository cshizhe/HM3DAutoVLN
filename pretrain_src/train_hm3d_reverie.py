...
def main(opts):
    ...
    for train_dataset_config in opts.train_datasets.values():
        model_config.pretrain_tasks.extend(train_dataset_config.get('tasks', []))
    ...
    data_cfg = EasyDict(opts.train_datasets['HM3D'])
    train_nav_db = ReverieTextPathData(
        ...
        obj_image_h=224, obj_image_w=224, scan_ranges=data_cfg.scan_ranges,
