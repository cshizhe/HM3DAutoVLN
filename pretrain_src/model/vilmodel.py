...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, 'share_scene_obj_enc'):
            self.share_scene_obj_enc = config.share_scene_obj_enc
        else:
            self.share_scene_obj_enc = not (config.obj_feat_size > 0 and config.obj_feat_size != config.image_feat_size)
       
        if self.share_scene_obj_enc:
