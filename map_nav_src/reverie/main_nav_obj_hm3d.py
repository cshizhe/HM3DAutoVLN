...
hm3d_dir = '../datasets/HM3D/nav_graphs_v1'
...
def build_dataset(args, rank=0):
    ...
    if not args.test:
        ...
        feat_db_file = os.path.join(hm3d_dir, 'features', 'view_timm_imagenet_vitb16')
        obj_db_file = os.path.join(hm3d_dir, 'features', 'obj2d_ade20k_pseudo3d_merged_timm_imagenet_vitb16')
    ...
    val_envs = {}
    if not args.test:
        val_envs['hm3d_train_seen'] = HM3DReverieObjectNavBatch(
            feat_db_file, obj_db_file, [aug_instr_file], args.connectivity_dir, 
            batch_size=args.batch_size, angle_feat_size=args.angle_feat_size, 
            image_feat_size=args.image_feat_size, obj_feat_size=args.obj_feat_size,
            max_objects=None, seed=args.seed+rank, sel_data_idxs=None, 
            name='hm3d_train_seen', multi_endpoints=False, multi_startpoints=False,
            scan_ranges=[0, 1]
        )
...
def main():
    args = parse_args()
    args.connectivity_dir = os.path.join(hm3d_dir, 'connectivity_hm3d_mp3d')
    args.aug = 'hm3d' if args.aug is None else args.aug
