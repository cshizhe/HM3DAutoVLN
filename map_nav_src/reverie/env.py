...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        ...
        if num_mp3d_scans is not None:
            ridxs = np.random.permutation(len(self.scans))[:num_mp3d_scans]
            self.scans = list(self.scans)
            self.scans = set([self.scans[ridx] for ridx in ridxs])
            data = []
            for item in self.data:
                if item['scan'] in self.scans:
                    data.append(item)
            self.data = data
            print('remained %d scans %d data' % (len(self.scans), len(self.data)))
    ...
    def _eval_item(self, scan, pred_path, pred_objid, gt_path, gt_objid):
        ...
        scores['rgs'] = (str(pred_objid) == str(gt_objid)) * scores['success']
